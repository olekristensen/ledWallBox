...
void testApp::setup()
{
    ...
    temperatureSpeed = 0.01;
    temperatureSpread = 0.01;
    temperatureTime = 0;
...
void testApp::setGUI(){
	...
	gui = new ofxUISuperCanvas("LEDlys Tesselation");
//    gui->addSpacer();
//    gui->addLabel("Press 'h' to Hide GUIs", OFX_UI_FONT_SMALL);

    gui->addSpacer();
	gui->addLabel("Temperature");
    ...
    gui->addSpacer();
    gui->addLabel("FPS", OFX_UI_FONT_SMALL);
...
void testApp::update()
{
    ...
    {
        TesselationSquare* t = *(it);
        float n = ofNoise(t->getX()*temperatureSpread, t->getY()*temperatureSpread, temperatureTime);
        //n = fmodf(temperatureTime, 1.);
        unsigned int temp = round(ofMap(n, 0, 1, fmaxf(t->kelvinWarm, kelvinWarmRange), fminf(t->kelvinCold, kelvinColdRange)));
        t->setTemperature(temp);

        float brightness = ofNoise(t->getX()*spread*0.1, t->getY()*spread*0.1, ofGetElapsedTimef()*speed*0.1);
   ...
   if (!ola_client.SendDmx(0, buffer))
    ...
    temperatureTime += powf(temperatureSpeed,5) * ( ( now - lastFrameSeconds ) / (1./60));
